...
class LaporanController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($tglAwal || $tglAkhir) {
            $awalValid  = $tglAwal  && \Carbon\Carbon::createFromFormat('Y-m-d', $tglAwal)  !== false;
            $akhirValid = $tglAkhir && \Carbon\Carbon::createFromFormat('Y-m-d', $tglAkhir) !== false;

            if (!$awalValid || !$akhirValid) {
                return redirect()->back()->with('filter_error', 'Filter tidak valid: format tanggal tidak sesuai.');
            }

            if ($tglAwal > $tglAkhir) {
                return redirect()->back()->with('filter_error', 'Filter tidak valid: tanggal akhir tidak boleh lebih kecil dari tanggal awal.');
            }
        }
        ...
        return view('admin.laporan', compact(
            ...
            'topPetani', 'tahunTersedia', 'tahun', 'bulan', 'dataPanen',
        ));
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        if (!$bulan) {
            return redirect()->route('admin.laporan')
                ->with('error', 'Pilih periode bulan terlebih dahulu sebelum export Excel.');
        }

        $periode = Carbon::create($tahun, $bulan)->locale('id')->isoFormat('MMMM Y');
        ...
        $namaFile = 'laporan-panen-' . $tahun . '-bulan' . str_pad($bulan, 2, '0', STR_PAD_LEFT) . '.csv';
        ...
        $callback = function () use ($dataPanen, $periode) {
            ...
            fputcsv($handle, []);
            fputcsv($handle, ['', '', '', '', 'TOTAL', '', $totalKg, round($totalKg / 1000, 2), '', '', '', $totalPendapatan]);
    ...
    public function exportPdf(Request $request)
    {
        ...
        if (!$bulan) {
            return redirect()->route('admin.laporan')
                ->with('error', 'Pilih periode bulan terlebih dahulu sebelum export PDF.');
        }
